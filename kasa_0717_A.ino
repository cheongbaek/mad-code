// ============================================================
//  A보드 : 인휠모터 좌우 독립 PID + 직접 PWM 테스트 (Arduino Mega 2560) - 0717 버전
//  kasa_0716_A.ino와 동일하되 입력 해석만 확장:
//    - "<값>" 단일(콤마 없음) : 무조건 펄스 모드 — 0~15만 유효, 그 외는 무시
//      (L스틱 조종/자율주행 노드의 기본 입력 경로. 0716과 동일한 해석)
//    - "<좌값>,<우값>" 콤마 2값 : 좌/우 바퀴에 아래 규칙을 각각 독립 적용 —
//        0~15   : 주행 목표펄스 (PID 주행, 좌우 따로 줄 수 있음)
//        16~255 : 직접 PWM — 받은 값을 그대로 analogWrite
//                 (PWM_MAX(170) 상한/슬루레이트/PID 전부 무시, 모터컨트롤러 특성 계측용)
//        256 이상 : '정지' (목표 0, 코스트 감속)
//      펄스/PWM 혼용 가능: 예 "2,7"=좌우 펄스 따로, "2,40"=왼쪽 펄스 2+오른쪽 PWM 40,
//                          "200,0"=왼쪽만 PWM 200 (조이스틱 A0/PWM 테스트 모드용)
//  직접 PWM -> 펄스 복귀 시 해당 바퀴 PID 상태 리셋 + 코스트 진입(캐치로 자연 재개)
//  그 외(좌우 독립 PID/FF보간/코스트-캐치/폭주감지/E-stop/텔레메트리)는 0716과 동일:
//    - 21번(왼쪽 펄스) → PID → 8번(왼쪽 PWM), 20번(오른쪽 펄스) → PID → 9번(오른쪽 PWM)
...
void setPulseTarget(uint8_t idx, int newTarget) {
  ...
  // 목표 하강 → 코스트 진입(무동력 감속), 상승 → 코스트 해제 (기존 0716 로직, 좌우 개별)
...
void updateWheel(unsigned long now) {
  ...
  int pwmL = (driveMode[LEFT] == DRIVE_PWM)
               ? pwmDirect[LEFT]
               : updatePid(LEFT,  target[LEFT],  wheel_speed21);   // 21번 펄스 -> 8번 PWM
  int pwmR = (driveMode[RIGHT] == DRIVE_PWM)
               ? pwmDirect[RIGHT]
               : updatePid(RIGHT, target[RIGHT], wheel_speed20);   // 20번 펄스 -> 9번 PWM
